...
public class FundReadDao implements IFundReadDao {
	...
	public List<Fund> queryFundList() {
		...
		try {
			...
			// String hql="from Fund a where not exists (select b.fundCode from
			// FundRateRpt b where b.fundCode=a.fundCode) order by
			// a.fundTypecode asc, a.fundCode asc";
			// Query query=session.createQuery(hql);

			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
			String hql = "from Fund a where a.rcdsts='A' and not exists (select b.fundCode from FundRateRpt b where b.fundCode=a.fundCode and (b.lstUpdDate>?)) order by a.fundTypecode asc, a.fundCode asc";
			Query query = session.createQuery(hql).setParameter(0, format.parse("2017-08-03"));
			fundList = query.list();
	...
	public List<Fund> queryFundList(String sCycleDate) throws Exception {
		...
		try {
			...
			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
			String hql = "from Fund a where a.rcdsts='A' and not exists (select b.fundCode from FundRateRpt b where b.fundCode=a.fundCode and (b.lstUpdDate>?)) order by a.fundTypecode asc, a.fundCode asc";
			Query query = session.createQuery(hql).setParameter(0, format.parse(sCycleDate));
			fundList = query.list();
		} finally {
			System.out.println("close session ..... ");
			HibernateSessionFactory.closeSession();
